...
class InviteePage extends Component {
      ...
      .then(state => {
        ...
        if (currentUserTimezone !== state.timezone) {
          ...
          state.selection.map(element => {
            var newElement = convertBetweenTimezones(
              element,
              state.prevInviteeTimezone,
              state.inviteeTimezone
            );
        ...
        Object.keys(row).forEach((key, i) => {
          ...
          if (date === parseInt(datetime)) {
            if (value === 1) inviteeSelection.push(key);
              ...
              inviteeSelection = inviteeSelection.filter(
  ...
  confirmTimes = () => {
    ...
    var newInviteeSelect = convertBetweenTimezones(
      inviteeSelection[0],
      inviteeTimezone,
      timezone
    );
  ...
  render() {
    ...
    return error ? (
      ...
      <div ref={this.viewportWidthRef} className="App">
        ...
        <InviteeTopBar
          inviteeDetailsHandler={this.inviteeDetailsHandler}
          inviteeTimezone={this.state.inviteeTimezone}
